...
export default class ProdutoDAO {
    ...
    async alterar(produto) {
        if (produto instanceof Produto) {
            const conexao = await conectar();
            const sql = `UPDATE produto SET prod_descricao=?,prod_precoCusto=?,prod_precoVenda=?,prod_qtdEstoque=?,prod_urlImagem=?,prod_dataValidade=str_to_date(?,'%Y/%m/%d'), prod_fk_codigo_cat = ?
                WHERE prod_codigo = ?
            `;
            let parametros = [
                produto.descricao,
